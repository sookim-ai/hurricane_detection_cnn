...
def train(sess,loss_op,train_op,X,Y_state_in,Y_lonlat_out,prediction_lonlat,last_state,iteration,path,fout_log):
    ...
    d1,d2,d3,d4,d5,t1,t2,t3,t4,t5,timesteps,train_size,val_size,channels,tr_image,va_image,tr_state_in,va_state_in,tr_lonlat_in,va_lonlat_in,tr_lonlat_out_out,va_lonlat_out_out=load_data(path)
    for it in range(iteration):
        ...
        for step in range(0,int(percent_of_ground_truth*num_epoch*train_size)):
            ...
            train=sess.run(train_op, feed_dict={X:image, Y_state_in:tr_state_in[step_i], Y_lonlat_out:tr_lonlat_out_out[step_i], init_state: zero_state})
            # Calculate batch loss in validation set
            if step>10 and step%100 == 0:
                val_sum=0
                for j in range(val_size):
                    image_va=mask_around_lonlat(va_image[j],va_lonlat_in[j])
                    lossv= sess.run(loss_op,  feed_dict={X:image_va, Y_state_in:va_state_in[j], Y_lonlat_out:va_lonlat_out_out[j], init_state: zero_state})
            ...
            # Phase 2
            if (it)*int(percent_of_ground_truth*num_epoch*train_size)+step % 1000 == 0 and it > 1 :
                step_i=np.random.randint(0,train_size);
                j=np.random.randint(0,val_size);
                time=0
                image=mask_around_lonlat(tr_image[step_i,:,time:time+1,:],tr_lonlat_in[step_i,:,time:time+1,:])
                feed_dict={X:image, Y_state_in:tr_state_in[step_i,:,time:time+1,:], Y_lonlat_out:tr_lonlat_out_out[step_i,:,time:time+1,:], init_state:zero_state}
                sess.run(train_op, feed_dict)
                eval_out=sess.run(fetches,feed_dict)
                next_state=eval_out['final_state']
                y_lonlat_in_out=eval_out['prediction_lonlat']
                y_lonlat_in=y_lonlat_in_out
                for time in xrange(1,d3-1):
                    image=mask_around_lonlat(tr_image[step_i,:,time:time+1,:],y_lonlat_in)
                    feed_dict={X:image, Y_state_in:tr_state_in[step_i,:,time:time+1,:], Y_lonlat_out:tr_lonlat_out_out[step_i,:,time:time+1,:],init_state:next_state}
                ...
                feed_dict={X:image_va, Y_state_in:va_state_in[j,:,time:time+1,:], Y_lonlat_out:va_lonlat_out_out[j,:,time:time+1,:],init_state:zero_state}
                eval_out=sess.run(fetches,feed_dict)
                next_state=eval_out['final_state']
                y_lonlat_in_out=eval_out['prediction_lonlat']
                y_lonlat_in=y_lonlat_in_out
                for time in xrange(1,timesteps):
                    ...
                    feed_dict={X:image, Y_state_in:va_state_in[j,:,time:time+1,:], Y_lonlat_out:va_lonlat_out_out[j,:,time:time+1,:],init_state:next_state}
